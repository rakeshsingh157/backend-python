...
@collaboration_bp.route("/api/<user_id>/collaboration/invite", methods=['POST'])
def invite_collaborator(user_id):
    inviter_id = user_id
    ...
    try:
        ...
        conn.commit()
        return jsonify({"message": "Invitation sent successfully!", "request_id": request_id}), 201
    except Error as e:
        return jsonify({"error": f"An internal error occurred: {e}"}), 500
    finally:
        if conn and conn.is_connected(): cursor.close(); conn.close()

@collaboration_bp.route("/api/<user_id>/collaboration/requests")
def get_collaboration_requests(user_id):
...
@collaboration_bp.route("/api/<user_id>/collaboration/requests/<int:request_id>", methods=['POST'])
def respond_to_request(user_id, request_id):
    current_user_id, action = user_id, request.json.get('action')
...
@collaboration_bp.route("/api/<user_id>/collaborators")
...
@collaboration_bp.route("/api/<user_id>/collaborator/remove", methods=['POST'])
...
@collaboration_bp.route("/api/<user_id>/tasks/personal")
...
@collaboration_bp.route("/api/<user_id>/tasks/assigned-by-me")
...
@collaboration_bp.route('/api/<user_id>/task/create_and_assign', methods=['POST'])
def create_and_assign_task(user_id):
    assigner_id = user_id
    data = request.json
...
@collaboration_bp.route('/api/<user_id>/task/<int:task_id>/toggle_done', methods=['POST'])
def toggle_task_done(user_id, task_id):
    conn = get_db_connection()
    if not conn: return jsonify({"error": "Database connection failed"}), 500
    try:
        cursor = conn.cursor()
...
@collaboration_bp.route('/api/<user_id>/task/<int:task_id>', methods=['DELETE'])
...
@collaboration_bp.route("/api/<user_id>/collaboration/events/month_view")
